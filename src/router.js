...
const CoachRegistation = defineAsyncComponent(() => import('./pages/requests/ContactCoach.vue'))
const ContactCoach = defineAsyncComponent(() => import('./pages/requests/RequestsReceived.vue'))
const RequestsReceived = defineAsyncComponent(() => import('./pages/NotFound.vue'))
...
router.beforeEach(function (to, _from, next) {
  if (to.meta.requiresAuth && !store.getters.isAuthenticated) {
    next('/auth')
  } else if (to.meta.requiresUnuth && store.getters.isAuthenticated) {
    next('/coaches')
  } else {
    next()
  }
